...
class qodeNewsClassWidgetLayout3 extends qodeNewsPhpClassWidget {
    public function __construct() {
        parent::__construct(
            'qode_layout3_widget',
            esc_html__('Qode Layout 3 Widget', 'qode-news'),
            array( 'description' => esc_html__( 'Display a layout 3', 'qode-news'))
        );
	...
	protected function setParams() {
        ...
        $params_post_item_array = qode_news_get_widget_params_from_VC(qode_news_get_post_item_shortcode_params(array('display_hot_trending_icons')));
		...
		$this->params = array_merge(
			...
			array(
	            array(
	                'type' => 'dropdown',
	                'title' => esc_html__('Display Review','qode-news'),
	                'name' => 'display_review',
	                'options' => array(
    ...
    public function widget($args, $instance) {
        ...
        echo '<div class="widget qode-news-widget qode-news-layout3-widget">';
		    if ( ! empty( $instance['widget_title'] ) ) {
			    echo wp_kses_post( $args['before_title'] ) . esc_html( $instance['widget_title'] ) . wp_kses_post( $args['after_title'] );
		    }

            echo qode_execute_shortcode('qode_layout3', $instance); // XSS OK
        echo '</div>';
    }
}